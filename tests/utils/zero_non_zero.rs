...
pub fn is_zero_gadget<CS: ConstraintSystem>(
    ...
    let one_minus_y_lc: LinearCombination = vec![(Variable::One(), FieldElement::from(1 - y))]
        .iter()
        .collect();
    let y_lc: LinearCombination = vec![(Variable::One(), FieldElement::from(y))]
        .iter()
        .collect();
    let inv_lc: LinearCombination = vec![(Variable::One(), FieldElement::from(inv))]
        .iter()
        .collect();
...
pub fn is_nonzero_gadget<CS: ConstraintSystem>(
    ...
    let one_lc: LinearCombination = vec![(Variable::One(), FieldElement::one())]
        .iter()
        .collect();
