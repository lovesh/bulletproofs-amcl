...
pub struct RangeProofProtocol<'a> {
    ...
    V: &'a GroupElement,
    size: usize,
    one_pow_vec: FieldElementVector,
    two_pow_vec: FieldElementVector
}
...
impl<'a> RangeProofProtocol<'a> {
    pub fn new(G: &'a GroupElementVector, H: &'a GroupElementVector, g: &'a GroupElement, h: &'a GroupElement,
        ...
        let size = G.len();
        let one_pow_vec = FieldElementVector::new_power_vector(&FieldElement::from(1), size);
        let two_pow_vec = FieldElementVector::new_power_vector(&FieldElement::from(2), size);
    ...
    pub fn gen_proof(&self, v: &FieldElement, lambda: &FieldElement) -> Result<RangeProof, ValueError> {
        ...
        let a_R = a_L.minus(&self.one_pow_vec)?;
        ...
        let z_sqr = z.square();
        let z_sqr_two = self.two_pow_vec.scaled_by(&z_sqr);
        let y_power_vector = FieldElementVector::new_power_vector(&y, self.size);

        // For debugging only, comment in production
        /*// <a_L, a_R o y^n> = 0
        let ar_yn = a_R.hadamard_product(&y_power_vector)?;
        ...
        let ip1 = a_L_minus1_minus_a_R.inner_product(&y_power_vector)?;
        ...
        let y_n_hadmard_a_R_plus_z_one = y_power_vector.hadamard_product(&a_R_plus_z_one)?;
        let r_X_const = y_n_hadmard_a_R_plus_z_one.plus(&z_sqr_two)?;
        // Linear coefficient of polynomial r(X)
        let r_X_linear = &y_power_vector.hadamard_product(&s_R)?;
        ...
        let t1 = add_field_elems!(&t1_1, &t1_2);
        ...
        let tau_x: FieldElement = add_field_elems!(&tau2_x_sqr, &tau1_x, &z_sqr_lambda);

        let mu: FieldElement = add_field_elems!(&alpha, &rho.multiply(&x));
        ...
        let P = add_group_elems!(&g_l, &h_r, &u_t_hat);
    ...
    pub fn verify_proof(&self, proof: &RangeProof) -> Result<bool, ValueError> {
        ...
        let mut rhs = add_group_elems!(&V_z_sqr, &g_delta, &T1_x, &T2_x_sqr);
        ...
        // z.y^n+z^2.2^n
        let y_power_vector = FieldElementVector::new_power_vector(&y, self.size);
        let z_y_n = y_power_vector.scaled_by(&z);
        let z_sqr = z.square();
        let z_sqr_two = self.two_pow_vec.scaled_by(&z_sqr);
        ...
        let P = add_group_elems!(&proof.A, &proof.S.scalar_multiplication(&x), &G_neg_z, &H_prime_exp);
        ...
        let mut newP = add_group_elems!(&P, &h_neg_mu);
        ...
        newP = add_group_elems!(&newP, &u_t_hat);
    ...
    fn calc_delta_y_z(&self, y: &FieldElement, z: &FieldElement) -> Result<FieldElement, ValueError>{
        ...
        let one_two_inner_product = self.two_pow_vec.sum();
        ...
        let one_y_inner_product = y_power_vector.sum();
