...
impl<'a> InnerProductArgument<'a> {
    pub fn new(
        ...
        u: &'a G1,
        P: &'a G1,
    ) -> Result<InnerProductArgument<'a>, ValueError> {
    ...
    ) -> Result<InnerProductArgumentProof, ValueError> {
        check_vector_size_for_equality!(a, b)?;
        let L = G1Vector::new(0);
        let R = G1Vector::new(0);
    ...
    pub fn verify_proof_recursively(
        &self,
        ...
        let mut state: Vec<u8> = vec![];
        self._verify_proof_recursively(
            &self.G, &self.H, &proof.L, &proof.R, &proof.a, &proof.b, &self.P, &mut state,
        )
    }
    ...
    fn _gen_proof(
        ...
        // Uncomment next line and comment above line for debugging
        //mut R: G1Vector, state: &mut Vec<u8>, P: &GroupElement) -> Result<InnerProductArgumentProof, ValueError> {
        match a.len() {
            1 => Ok(InnerProductArgumentProof {
                L,
                R,
                a: a[0].clone(),
                b: b[0].clone(),
            ...
            n => {
                ...
                self._gen_proof(&g_prime, &h_prime, &a_prime, &b_prime, L, R, state)
                // Uncomment next line and comment above line for debugging
                //L, R, state, &P_prime)
            }
        }
    ...
    fn _verify_proof_recursively(
        ...
        match g.len() {
            ...
            n => {
                ...
                let newL = G1Vector::from(newL);
                let (_, newR) = R.as_slice().split_at(1);
                let newR = G1Vector::from(newR);
                self._verify_proof_recursively(
                    &g_prime, &h_prime, &newL, &newR, a, b, &P_prime, state,
                )
            }
        }
    }
    ...
    fn hash(
        g: &G1Vector,
        h: &G1Vector,
        a1: &FieldElementVector,
        a2_prime: &FieldElementVector,
        b1: &FieldElementVector,
        b2_prime: &FieldElementVector,
        u: &G1,
        c: &FieldElement,
    ...
    // Construct H' = H^(y^-n)
    pub fn compute_h_prime(H: &G1Vector, y: &FieldElement) -> G1Vector {
        let y_inv = y.inverse();
        let y_inv_vandm_vec = FieldElementVector::new_vandermonde_vector(&y_inv, H.len());

        // Construct H' = H^(y^-n)
        let mut H_prime = G1Vector::with_capacity(H.len());
        for i in 0..H.len() {
...
mod test {
    ...
    fn test_inner_product_argument() {
        ...
        // Incorrect proof
        let d: FieldElementVector = vec![6, 6, 7, 8]
            .iter()
            .map(|i| FieldElement::from(*i as u8))
            .collect::<Vec<FieldElement>>()
            .into();
